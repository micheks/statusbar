...
public class RequestManagerRetriever  implements Handler.Callback {
    private String mTag = StatusBarUtils.class.getName();
     ...
     */
    public StatusBarUtils get(Activity activity) {
        checkNotNull(activity, "activity is null");
        String tag = mTag + System.identityHashCode(activity);
        if (activity instanceof FragmentActivity) {
            return getSupportFragment(((AppCompatActivity) activity).getSupportFragmentManager(), tag).get(activity);
        } else {
            return getFragment(((FragmentActivity) activity).getSupportFragmentManager(), tag).get(activity);
        }
    }
     ...
     * @return the immersion bar
     */
    public StatusBarUtils get(Fragment fragment, boolean isOnly) {
...
//    }
    ...
    public StatusBarUtils get(Activity activity, Dialog dialog) {
        ...
        if (activity instanceof FragmentActivity) {
            ...
            return getFragment(((FragmentActivity) activity).getSupportFragmentManager(), tag).get(activity, dialog);
        }
    }
    ...
    public void destroy(Activity activity, Dialog dialog) {
        ...
        if (activity instanceof FragmentActivity) {
            ...
            RequestManagerFragment fragment = getFragment(((FragmentActivity) activity).getSupportFragmentManager(), tag, true);
            if (fragment != null) {
                fragment.get(activity, dialog).onDestroy();
            }
        }
